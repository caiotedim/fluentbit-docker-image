...
RUN echo "deb http://deb.debian.org/debian bookworm-backports main" >> /etc/apt/sources.list && \
    ...
    && mkdir /src \
    && curl -L https://github.com/fluent/fluent-bit/archive/refs/tags/v${RELEASE_VERSION}.tar.gz -o fluent-bit-${RELEASE_VERSION}.tar.gz \
    && tar -xzf fluent-bit-${RELEASE_VERSION}.tar.gz -C /src \
    && rm -rf fluent-bit-${RELEASE_VERSION}.tar.gz

# Must be run from root of repo
WORKDIR /src/fluent-bit-${RELEASE_VERSION}/
COPY . ./

# We split the builder setup out so people can target it or use as a base image without doing a full build.
FROM builder-base as builder
WORKDIR /src/fluent-bit-${RELEASE_VERSION}/build/
